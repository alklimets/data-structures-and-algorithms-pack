package com.aklimets.data_structures.graphs.leetcode.course_schedule4;
    ...
    */
    ...
    public List<Boolean> checkIfPrerequisite(int numCourses, int[][] prerequisites, int[][] queries) {
        ...
        for (int i = 0; i < queries.length; i++) {
            int[] query = queries[i];
            res.add(dfs(query[0], query[1], graph));
        }
        return res;
    }

    private boolean dfs(int start, int target, Map<Integer, List<Integer>> graph) {
        if (memo.getOrDefault(start, Set.of()).contains(target)) return true;
        if (start == target) {
            return true;
        }

        for (int rel : graph.getOrDefault(start, List.of())) {
            if (dfs(rel, target, graph)) {
                memo.computeIfAbsent(rel, v -> new HashSet<>()).add(target);
                return true;
            }
        }
        return false;
